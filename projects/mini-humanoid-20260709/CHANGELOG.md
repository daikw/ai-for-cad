...
単独デザイナー（Fable）による一貫デザインパス。v2 の「角丸箱の集合」感の解消。
チェックは **97 passed / 6 waived / 0 failed**（--backend occt）、総質量 666.19g。

- **胴体を 2 段構成に再設計**: 暗色の絞れたウエスト（60×42 → 72×46 のテーパー押し出し）
  + 白い胸ブロック。ref-1 の台形シルエットは「胸を細くする」のではなく「ウエストを
  絞る」ことで実現（肩サーボ用キャビティ ±33 が胸幅 72 を固定するため）。
  電装（バッテリー・基板）は凍結位置のまま、ウエスト内キャビティ 54×39.5 に収まる
- **ヒップスカート**: 骨盤側面に白パネルを追加し股関節の機械感を緩和
  （hip roll サーボ y≤-14 と 1mm クリアランスで共存）
- **脚シェルをテーパー押し出し化**: すね 32→36、大腿 32→38 のフレア。
  fillet 不可のテーパー押し出しでは丸みを roundedRect の角 R で作る
- **バイザー大型化・角丸化**: 54×26 の roundedRect 窓（v2 は 43×22 の角張った box 窓）、
  目を拡大、耳ポッド Ø26 化
- デザイン言語を「roundedRect + scaleTop テーパー押し出し」に統一
  （fillet 依存を削減 → truck/occt 両対応が単純化）
