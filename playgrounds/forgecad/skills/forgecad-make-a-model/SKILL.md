...
6. Verify against the design doc — if the model has an LLD or is a multi-part assembly, build/refresh `checks.forge.js` with the `/forgecad-verify` skill and run it with `--backend occt`. The task is complete only when the suite reports ALL PASS (waived items listed). Numeric checks are the acceptance gate; the Render-Verify Loop below is the visual complement, not the gate.
...
You are building blind unless you render. `forgecad run` only checks that code executes — it cannot tell you a hole is in the wrong place or a part doesn't fit. Render and look at the result. For the numeric acceptance gate (interference, clearance, mass budget, bed fit), use `/forgecad-verify` instead — this loop is the visual complement, not a substitute.
...
Run with `node shared-dims.js` to see the full dimension summary. Don't write throwaway `node -e "require(...)..."` scripts — put the inspection logic in the source file itself where it stays up to date automatically. Mark unmeasured placeholder values (e.g. motor hole PCD before the SKU is fixed) with an UNKNOWN/placeholder comment so the verification suite can list them as waived.
